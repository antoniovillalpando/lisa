(async function () {
  ...
  function update() {
    ...
    const domain = model.dominios[activeDomain];
    const domainLabel = domain.dominio;
    const promedioNacional = domain.promedio_nacional;

    const prediccionPerfil = predict(p);
    const prediccionReferencia = predict(ref);
    const y = promedioNacional + (prediccionPerfil - prediccionReferencia);
    const diferencia = y - promedioNacional;
    ...
    if (Math.abs(diferencia) < 0.5) {
      els.comparison.textContent = `Promedio nacional de México en ${domainLabel.toLowerCase()}.`;
    } else {
      const direccion = diferencia > 0 ? 'por encima' : 'por debajo';
      els.comparison.textContent = `${Math.abs(Math.round(diferencia))} puntos ${direccion} del promedio nacional de México en ${domainLabel.toLowerCase()}.`;
    }
  }
  ...
  function reset() {
    ...
    els.ampliacion.value = ref.ia_ampliacion;
    els.sustitucion.value = ref.ia_sustitucion;
    update();
  }
