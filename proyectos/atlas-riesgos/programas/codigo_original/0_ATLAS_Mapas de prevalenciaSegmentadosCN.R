...
library(ggplot2)
library(writexl)
library(ggrepel)  # Añadido para geom_text_repel
...
for (grupo in names(resultados)) {
  ...
  desnutricion_plot <- ggplot(mapa_y_datos) +
    ...
    geom_text_repel(aes(label = paste0(round(porcentaje_bajo_umbral, 1), "%"), 
                        geometry = geometry),
  ...
  obesidad_plot <- ggplot(mapa_y_datos2) +
    ...
    scale_fill_gradient(low = "#E7E7FF", high = "#0010C3", name = "Porcentaje") +
    geom_text_repel(aes(label = paste0(round(porcentaje_sobre_umbral, 1), "%"), 
                        geometry = geometry),
                    stat = "sf_coordinates", size = 4, fontface = "bold", 
                    nudge_x = 0.15, nudge_y = 0.15, max.overlaps = Inf,
                    segment.color = "grey50", segment.size = 0.5) +
